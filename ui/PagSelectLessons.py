...
class PagSelectLessons(PagSelectBase):
    ...
    def getSelectOptionsPanel(self):
        ...
        ##
        lbl = SortLabel(panel)
        lbl.Caption = biz.getColCaption("ShortName")
        lbl.relatedDataField = "ShortName"
        ...
        if ctrlClass is not None:
            ...
            # Store the info for later use when constructing the query
            self.selectFields["ShortName"] = {
                    "ctrl" : ctrl,
                    "op" : opList,
                    "type": "char"
                    }
        else:
            dabo.log.error("No control class found for field 'ShortName'.")
            lbl.safeDestroy()
            opList.safeDestroy()
...
if __name__ == "__main__":
    ...
    frm = TestForm(Caption="Test Of PagSelectLessons", Testing=True)
    test = PagSelectLessons(frm)
    test.createItems()
